 ...
 * <p/>
 * This type of String will usually only be used when it is the last field
 * within a frame, when reading the remainder of the byte array will be read,
 * when writing the frame will be accomodate the required size for the String.
 * The String will be encoded based upon the text encoding of the frame that it
 * belongs to.
 *
 * All TextInformation frames support multiple strings, stored as a null
 * separated list, where null is represented by the termination code for the
 * character encoding. This functionality is only officially support in ID3v24.
 * Itunes write null terminators characters even though only writes a single
 * value.
 */
public class TextEncodedStringSizeTerminated extends AbstractString {
	...
	/**
	 * Read a 'n' bytes from buffer into a String where n is the framesize -
	 * offset so thefore cannot use this if there are other objects after it
	 * because it has no delimiter.
	 * <p/>
	...
	public byte[] writeByteArray() {
		byte[] data = null;
		// Try and write to buffer using the CharSet defined by
	 ...
	 *
	 * The text encoding is defined by the frame body that the text field
	 * belongs to.
	 *
	 ...
	 */
	private static List<String> splitByNullSeperator(String value) {
