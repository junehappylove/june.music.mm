 ...
 *
 * The String itself might be of length zero (i.e just consist of the null
 * character). The String will be encoded based upon the text encoding of the
 * frame that it belongs to.
 */
public class TextEncodedStringNullTerminated extends AbstractString {
	...
	public void readByteArray(byte[] arr, int offset) throws InvalidDataTypeException {
		...
		// We only want to load up to null terminator, data after this is part
		// of different
		// field and it may not be possible to decode it so do the check before
		// we do
		...
		final boolean nullIsOneByte = (charSetName.equals(TextEncoding.CHARSET_ISO_8859_1)
				|| charSetName.equals(TextEncoding.CHARSET_UTF_8));
				...
				if (nullIsOneByte) {
					...
					if (buffer.hasRemaining()) {
						...
						if (nextByte == 0x00) {
							...
							// Nothing to do, we have checked 2nd value of pair
							// it was not a null terminator
							// so will just start looking again in next
							// invocation of loop
						}
					} else {
						buffer.mark();
		...
		// Set Size so offset is ready for next field (includes the null
		// terminator)
		...
		// catch and then set value to empty string. (We don't read the null
		// terminator
	...
	/**
	 * Write String into byte array, adding a null character to the end of the
	 * String
	 *
