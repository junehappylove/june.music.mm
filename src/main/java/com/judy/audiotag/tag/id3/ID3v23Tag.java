...
import com.judy.audiotag.tag.id3.framebody.FrameBodyTDAT;
import com.judy.audiotag.tag.id3.framebody.FrameBodyTDRC;
import com.judy.audiotag.tag.id3.framebody.FrameBodyTIME;
import com.judy.audiotag.tag.id3.framebody.FrameBodyTYER;
...
public class ID3v23Tag extends AbstractID3v2Tag {
	 ...
	 */
	private static final long serialVersionUID = -8040904102056347750L;
	...
	protected void copyFrames(AbstractID3v2Tag copyObject) {
		...
		while (iterator.hasNext()) {
			...
			if (o instanceof AbstractID3v2Frame) {
				...
				// only convert when
				// it is a valid tdrc to cope with tdrc being added illegally to
				// a v23 tag which is then
			...
			// Multi Frames
				...
				ArrayList<ID3v23Frame> multiFrame = new ArrayList<ID3v23Frame>();
	...
	/**
	 * This is used when we need to translate a single frame into multiple
	 * frames, currently required for v24 TDRC frames.
	 */
	...
	public ID3v23Tag(AbstractTag mp3tag) {
		...
		if (mp3tag != null) {
			...
			// All tags types can be converted to v2.4 so do this to simplify
			// things
			else {
	...
	/**
	 * Return frame size based upon the sizes of the tags rather than the
	 * physical no of bytes between start of ID3Tag and start of Audio Data.
	 *
	...
	public int getSize() {
		...
		if (extended) {
			size += ID3v23Tag.TAG_EXT_HEADER_LENGTH;
			if (crcDataFlag) {
				size += ID3v23Tag.TAG_EXT_HEADER_CRC_LENGTH;
			}
		}
		size += super.getSize();
		return size;
	...
	public void read(ByteBuffer buffer) throws TagException {
		...
		logger.info(getLoggingFilename() + ":Tag size is:" + size
				+ " according to header (does not include header size, add 10)");
	...
	protected void readFrames(ByteBuffer byteBuffer, int size) {
		...
		logger.finest(getLoggingFilename() + ":Start of frame body at:" + byteBuffer.position()
				+ ",frames data size is:" + size);

		// Read the frames until got to upto the size as specified in header or
		// until
		...
		while (byteBuffer.position() < size) {
			...
			// Problem trying to find frame, often just occurs because
			// frameheader includes padding
	...
	private ByteBuffer writeHeaderToBuffer(int padding, int size) throws IOException {
		...
		if (extended) {
			additionalHeaderSize += ID3v23Tag.TAG_EXT_HEADER_LENGTH;
			if (crcDataFlag) {
				additionalHeaderSize += ID3v23Tag.TAG_EXT_HEADER_CRC_LENGTH;
			}
		}
	 ...
	 * Write tag to file
	 *
	 * TODO:we currently never write the Extended header , but if we did the
	 * size calculation in this method would be slightly incorrect
	 *
	 * @param file
