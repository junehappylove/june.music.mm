...
import com.judy.audiotag.audio.mp3.MPEGFrameHeader;
...
public class ID3Unsynchronization {
	...
	/**
	 * Check if a byte array will require unsynchronization before being written
	 * as a tag. If the byte array contains any $FF $E0 bytes, then it will
	 * require unsynchronization.
	 *
	 * @param abySource
	...
	/**
	 * Unsynchronize an array of bytes, this should only be called if the
	 * decision has already been made to unsynchronize the byte array
	 * <p/>
	 * In order to prevent a media player from incorrectly interpreting the
	 * contents of a tag, all $FF bytes followed by a byte with value >=224 must
	 * be followed by a $00 byte (thus, $FF $F0 sequences become $FF $00 $F0).
	 * Additionally because unsynchronisation is being applied any existing $FF
	 * $00 have to be converted to $FF $00 $00
	 *
	 * @param abySource
	...
	public static byte[] unsynchronize(byte[] abySource) {
		...
		while (oBAIS.available() > 0) {
			...
			if ((iVal & MPEGFrameHeader.SYNC_BYTE1) == MPEGFrameHeader.SYNC_BYTE1) {
				// if byte is $FF, we must check the following byte if there is
				// one
				if (oBAIS.available() > 0) {
					oBAIS.mark(1); // remember where we were, if we don't need
									// to unsynchronize
		...
		// if we needed to unsynchronize anything, and this tag ends with 0xff,
		// we have to append a zero byte,
	...
	/**
	 * Synchronize an array of bytes, this should only be called if it has been
	 * determined the tag is unsynchronised
	 * <p/>
