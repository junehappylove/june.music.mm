...
/**
 * This contains static methods that can be performed on tags and to convert
 * between tags.
 * 
 * @author : Paul Taylor
 * @author : Eric Farng
...
public class ID3Tags {
	 ...
	 * parse a string, and takes <code>Long, Short, Byte, Integer</code> objects
	 * and gets their value. An exception is not explicitly thrown here because
	 * it would causes too many other methods to also throw it.
	 *
	 * @param value
	...
	public static String convertFrameID22To24(String identifier) {
		...
		if (id != null) {
			...
			if (v23id == null) {
				// if not it may be because v2.3 and and v2.4 are same so wont
				// be
				// in mapping
	...
	public static String convertFrameID23To22(String identifier) {
		...
		if (ID3v23Frames.getInstanceOf().getIdToValueMap().containsKey(identifier)) {
			// If only name has changed v22 and modified in v23 return result
			// of.
	...
	public static String convertFrameID23To24(String identifier) {
		...
		if (ID3v23Frames.getInstanceOf().getIdToValueMap().containsKey(identifier)) {
			...
			// If only name has changed ID3v23 and modified in ID3v24 return
			// result of.
			else {
	...
	/**
	 * Force from ID3v22 FrameIdentifier to ID3v23, this is where the frame and
	 * structure has changed from v2 to v3 but we can still do some kind of
	 * conversion.
	 */
	...
	/**
	 * Force from ID3v2.30 FrameIdentifier to ID3v2.40, this is where the frame
	 * and structure has changed from v3 to v4 but we can still do some kind of
	 * conversion.
	 */
	...
	/**
	 * Force from ID3v2.40 FrameIdentifier to ID3v2.30, this is where the frame
	 * and structure has changed between v4 to v3 but we can still do some kind
	 * of conversion.
	 */
