...
import com.judy.audiotag.tag.id3.framebody.ID3v23FrameBody;
...
public class ID3v23Frame extends AbstractID3v2Frame {
	 ...
	 */
	private static final long serialVersionUID = 6884767034486810450L;
	 ...
	 *
	 * <p>
	 * An empty body of the correct type will be automatically created. This
	 * constructor should be used when wish to create a new frame from scratch
	 * using user data.
	 */
		...
		if (frame instanceof ID3v24Frame) {
			if (ID3Tags.isID3v24FrameIdentifier(frame.getIdentifier())) {
				...
				if (identifier != null) {
					...
					// Is it a known v4 frame which needs forcing to v3 frame
					// e.g. TDRC - TYER,TDAT
					...
					// It is a v24 frame that is not known and cannot be forced
					// in v23 e.g TDRL,in which case
					// we convert to a framebody unsupported by writing contents
					// as a byte array and feeding
			...
			if (ID3Tags.isID3v22FrameIdentifier(frame.getIdentifier())) {
				...
				// Is it a known v2 frame which needs forcing to v4 frame e.g
				// PIC - APIC
					...
					// No mechanism exists to convert it to a v23 frame
						...
						identifier = frame.getIdentifier();
						logger.info(
								"Deprecated:V22:orig id id is:" + frame.getIdentifier() + ":New id is:" + identifier);
						return;
					}
				}
			}
	...
	/**
	 * Compare for equality To be deemed equal obj must be a IDv23Frame with the
	 * same identifier and the same flags. containing the same body,datatype
	 * list ectera. equals() method is made up from all the various components
	 *
	 * @param obj
	...
	public void read(ByteBuffer byteBuffer) throws InvalidFrameException {
		...
		// Read the size field (as Big Endian Int - byte buffers always
		// initlised to Big endian order)
		...
		// If this identifier is a valid v24 identifier or easily converted to
		// v24
		...
		logger.fine(getLoggingFilename() + ":Identifier was:" + identifier + " reading using:" + id + "with frame size:"
				+ frameSize);
		...
		if (((EncodingFlags) encodingFlags).isCompression()) {
			// Decompress the bytes into this buffer, size initialized from
			// header field
			...
			// Read frame data into array, and then put buffer back to where it
			// was
			...
			try {
				...
				dfe.printStackTrace();
				// Update position of main buffer, so no attempt is made to
				// reread these bytes
				byteBuffer.position(byteBuffer.position() + realFrameSize);
				throw new InvalidFrameException(getLoggingFilename() + "Unable to decompress this frame:" + identifier
						+ ":" + dfe.getMessage());
			}
			decompresser.end();
		...
		try {
			if (((EncodingFlags) encodingFlags).isCompression()) {
				...
				// Create Buffer that only contains the body of this frame
				// rather than the remainder of tag
				frameBodyBuffer = byteBuffer.slice();
				frameBodyBuffer.limit(realFrameSize);
				frameBody = readBody(id, frameBodyBuffer, realFrameSize);
			}
			// TODO code seems to assume that if the frame created is not a
			// v23FrameBody
			// it should be deprecated, but what about if somehow a V24Frame has
			// been put into a V23 Tag, shouldnt
			// it then be created as FrameBodyUnsupported
			if (!(frameBody instanceof ID3v23FrameBody)) {
				logger.info(
						getLoggingFilename() + ":Converted frame body with:" + identifier + " to deprecated framebody");
	...
	/**
	 * This represents a frame headers Status Flags Make adjustments if
	 * necessary based on frame type and specification.
	 */
