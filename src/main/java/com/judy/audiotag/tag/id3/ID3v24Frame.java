...
import com.judy.audiotag.FileConstants;
...
import com.judy.audiotag.tag.datatype.Lyrics3Line;
...
import com.judy.audiotag.tag.id3.framebody.FrameBodySYLT;
import com.judy.audiotag.tag.id3.framebody.FrameBodyTALB;
import com.judy.audiotag.tag.id3.framebody.FrameBodyTCOM;
import com.judy.audiotag.tag.id3.framebody.FrameBodyTIT2;
import com.judy.audiotag.tag.id3.framebody.FrameBodyTPE1;
import com.judy.audiotag.tag.id3.framebody.FrameBodyUSLT;
import com.judy.audiotag.tag.id3.framebody.FrameBodyUnsupported;
import com.judy.audiotag.tag.id3.framebody.ID3v24FrameBody;
import com.judy.audiotag.tag.lyrics3.FieldFrameBodyAUT;
import com.judy.audiotag.tag.lyrics3.FieldFrameBodyEAL;
import com.judy.audiotag.tag.lyrics3.FieldFrameBodyEAR;
import com.judy.audiotag.tag.lyrics3.FieldFrameBodyETT;
import com.judy.audiotag.tag.lyrics3.FieldFrameBodyINF;
import com.judy.audiotag.tag.lyrics3.FieldFrameBodyLYR;
import com.judy.audiotag.tag.lyrics3.Lyrics3v2Field;
...
public class ID3v24Frame extends AbstractID3v2Frame {
	...
	/**
	 * Creates a new ID3v2_4Frame of type identifier. An empty body of the
	 * correct type will be automatically created. This constructor should be
	 * used when wish to create a new frame from scratch using user input
	 *
	 * @param identifier
	...
	/**
	 * Copy Constructor:Creates a new ID3v2_4Frame datatype based on another
	 * frame.
	 */
	...
	private void createV24FrameFromV23Frame(ID3v23Frame frame) throws InvalidFrameException {
		...
		// Is it a known v3 frame which needs forcing to v4 frame e.g. TYER -
		// TDRC
			...
			// No mechanism exists to convert it to a v24 frame, e.g deprecated
			// frame e.g TSIZ, so hold
	...
	/**
	 * Creates a new ID3v2_4Frame datatype based on another frame of different
	 * version Converts the framebody to the equivalent v24 framebody or to
	 * UnsupportedFrameBody if identifier is unknown.
	 *
	 * @param frame
	...
	public ID3v24Frame(AbstractID3v2Frame frame) throws InvalidFrameException {
		...
		// version we should be able to convert it to an v24 frame, although it
		// may mean minor
		// modification to the data. If it was not recognised originally it
		// should remain
		// unknown.
		if (frame instanceof ID3v23Frame) {
	...
	/**
	 * Read the frame from the specified file. Read the frame header then
	 * delegate reading of data to frame body.
	 *
	 * @param byteBuffer
	...
	public void read(ByteBuffer byteBuffer) throws InvalidFrameException {
		...
		if (frameSize < 0) {
			...
			logger.warning(
					getLoggingFilename() + ":" + "Invalid Frame size larger than size before mp3 audio:" + identifier);
			throw new InvalidFrameException(identifier + " is invalid frame");
		}

		if (frameSize > ID3SyncSafeInteger.MAX_SAFE_SIZE) {
			// Set Just after size field this is where we want to be when we
			// leave this if statement
			...
			// Is the frame size syncsafe, should always be BUT some encoders
			// such as Itunes do not do it properly
			...
			if (isNotSyncSafe) {
				logger.warning(
						getLoggingFilename() + ":" + "Frame size is NOT stored as a sync safe integer:" + identifier);

				// This will return a larger frame size so need to check against
				// buffer size if too large then we are
				// buggered , give up
				if (nonSyncSafeFrameSize > (byteBuffer.remaining() - -FRAME_FLAGS_SIZE)) {
					logger.warning(getLoggingFilename() + ":" + "Invalid Frame size larger than size before mp3 audio:"
							+ identifier);
					throw new InvalidFrameException(identifier + " is invalid frame");
				} else {
				...
				// appears to be sync safe but lets look at the bytes just after
				// the reported end of this
				...
				if (byteBuffer.remaining() < FRAME_ID_SIZE) {
					// There is no padding or framedata we are at end so assume
					// syncsafe
					...
					if (isValidID3v2FrameIdentifier(readAheadIdentifier)) {
						...
						// no data found so assume entered padding in which case
						// assume it is last
						// frame and we are ok
					}
					// havent found identifier so maybe not syncsafe or maybe
					// there are no more frames, just padding
					else {
						...
						if (nonSyncSafeFrameSize > byteBuffer.remaining() - FRAME_FLAGS_SIZE) {
							...
							if (byteBuffer.remaining() >= FRAME_ID_SIZE) {
								...
								// ok found a valid identifier using
								// non-syncsafe so assume non-syncsafe size
								// and continue
								if (isValidID3v2FrameIdentifier(readAheadIdentifier)) {
									frameSize = nonSyncSafeFrameSize;
									logger.warning(getLoggingFilename() + ":"
											+ "Assuming frame size is NOT stored as a sync safe integer:" + identifier);
								}
								// no data found so assume entered padding in
								// which case assume it is last
								// frame and we are ok whereas we didnt hit
								// padding when using syncsafe integer
								// or we wouldnt have got to this point. So
								// assume syncsafe ineteger ended within
								// the frame data whereas this has reached end
								// of frames.
								else if (ID3SyncSafeInteger.isBufferEmpty(readAheadbuffer)) {
									frameSize = nonSyncSafeFrameSize;
									logger.warning(getLoggingFilename() + ":"
								...
								// invalid so assume syncsafe as that is is the
								// standard
								else {
									;
								}
								...
								// If the unsync framesize matches exactly the
								// remaining bytes then assume it has the
		...
		// These are not included in header size but are included in frame size
		// but wont be read when we actually
		...
		if (((EncodingFlags) encodingFlags).isDataLengthIndicator()) {
			// There is only data length indicator it may have already been read
			// depending on what flags
			// are set
			if (!isDataLengthindicatorRead) {
				...
				logger.info(getLoggingFilename() + ":" + "Frame Size Is:" + frameSize + "Data Length Size:"
						+ datalengthSize);
			}
		}
		...
		// Create Buffer that only contains the body of this frame rather than
		// the remainder of tag
		...
		if (((EncodingFlags) encodingFlags).isUnsynchronised()) {
			// We only want to synchronize the buffer upto the end of this frame
			// (remember this
			// buffer contains the remainder of this tag not just this frame),
			// and we cant just
			// create a new buffer because when this method returns the position
			// of the buffer is used
			// to look for the next frame, so we need to modify the buffer. The
			// action of synchronizing causes
			// bytes to be dropped so the existing buffer is large enough to
			// hold the modifications
		...
		try {
			...
			if (!(frameBody instanceof ID3v24FrameBody)) {
				logger.info(getLoggingFilename() + ":" + "Converted frame body with:" + identifier
						+ " to deprecated framebody");
				frameBody = new FrameBodyDeprecated((AbstractID3v2FrameBody) frameBody);
			}
		} finally {
			// Update position of main buffer, so no attempt is made to reread
			// these bytes
	...
	/**
	 * Write the frame. Writes the frame header but writing the data is
	 * delegated to the frame body.
	 *
	 * @throws IOException
	 */
	public void write(ByteArrayOutputStream tagBuffer) {
		...
		// Write Frame Header
		// Write Frame ID, the identifier must be 4 bytes bytes long it may not
		// be
		...
		// Write Frame Size based on size of body buffer (if it has been
		// unsynced then it size
		...
		try {
			...
			// This could never happen coz not writing to file, so convert to
			// RuntimeException
	...
	/**
	 * Member Class This represents a frame headers Status Flags Make
	 * adjustments if necessary based on frame type and specification.
	 */
	class StatusFlags extends AbstractID3v2Frame.StatusFlags {
		...
		public void createStructure() {
			...
			MP3File.getStructureFormatter().addElement(TYPE_TAGALTERPRESERVATION,
					originalFlags & MASK_TAG_ALTER_PRESERVATION);
			MP3File.getStructureFormatter().addElement(TYPE_FILEALTERPRESERVATION,
	...
	class EncodingFlags extends AbstractID3v2Frame.EncodingFlags {
		...
		/**
		 * Use this when creating a frame from existing flags in another v4
		 * frame
		 */
		...
		public void createStructure() {
			...
			MP3File.getStructureFormatter().addElement(TYPE_FRAMEUNSYNCHRONIZATION,
					flags & MASK_FRAME_UNSYNCHRONIZATION);
	 ...
	 * must start with a capital letter and only contain capital letters and
	 * numbers
	 *
	 * @param identifier
	 *            to be checked
