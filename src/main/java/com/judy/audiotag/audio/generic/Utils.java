...
public class Utils {
	...
	/**
	 * Copies the bytes of <code>srd</code> to <code>dst</code> at the specified
	 * offset.
	 *
	 * @param src
	...
	/*
	 * Computes a number whereby the 1st byte is the least signifcant and the
	 ...
	 *
	 * So if storing a number which only requires one byte it will be stored in
	 * the first byte.
	 */
	...
	/*
	 * Computes a number whereby the 1st byte is the most significant and the
	...
	/**
	 * Convert int to byte representation - Little Endian (as used by ogg
	 * vorbis)
	 *
	 * @param size
	 * @return byte represenetation
	...
	/**
	 * Create String offset from position by offset upto length using encoding,
	 * and position of buffer is moved to after position + offset + length
	 *
	 * @param buffer
	 * @param offset
	 * @param length
