 ...
 *
 * <p>
 * The audio header consists of a number of audio frames. Because we are not
 * trying to play the audio but only extract some information regarding the
 * audio we only need to read the first audio frames to ensure that we have
 * correctly identified them as audio frames and extracted the metadata we
 * reuire.
 * 
 * <p>
 * Start of Audio id 0xFF (11111111) and then second byte anded with
 * 0xE0(11100000). For example 2nd byte doesnt have to be 0xE0 is just has to
 * have the top 3 signicant bits set. For example 0xFB (11111011) is a common
 * occurence of the second match. The 2nd byte defines flags to indicate various
 * mp3 values.
 *
 * <p>
 * Having found these two values we then read the header which comprises these
 * two bytes plus a further two to ensure this really is a MP3Header, sometimes
 * the first frame is actually a dummy frame with summary information held
 * within about the whole file, typically using a Xing Header or LAme Header.
 * This is most useful when the file is variable bit rate, if the file is
 * variable bit rate but does not use a summary header it will not be correctly
 * identified as a VBR frame and the track length will be incorreclty
 * calculated. Strictly speaking MP3 means an MPEG-1, Layer III file but MP2
 * (MPEG-1,Layer II), MP1 (MPEG-1,Layer I) and MPEG-2 files are sometimes used
 * and named with the .mp3 suffix so this library attempts to supports all these
 * formats.
 */
public final class MP3AudioHeader implements AudioHeader {
	 ...
	 * bytes so this is why chosen as a default.
	 */
	 ...
	 *
	 * The search starts from the start of the file, it is usually safer to use
	 * the alternative constructor that allows you to provide the length of the
	 * tag header as a parameter so the tag can be skipped over.
	 *
	 * @param seekFile
	 * @throws IOException
	 ...
	 * Search for the first MP3Header in the file
	 *
	 * Starts searching from location startByte, this is because there is likely
	 * to be an ID3TagHeader before the start of the audio. If this tagHeader
	 * contains unsynchronized information there is a possibility that it might
	 * be inaccurately identified as the start of the Audio data. Various checks
	 * are done in this code to prevent this happening but it cannot be
	 * guaranteed.
	 *
	 * Of course if the startByte provided overstates the length og the tag
	 * header, this could mean the start of the MP3AudioHeader is missed,
	 * further checks are done within the MP3 class to recognize if this has
	 ...
	 *            MP3 file to seek
	 * @param startByte
	 *            if there is an ID3v2tag we dont want to start reading from the
	 *            start of the tag
	...
	public boolean seek(final File seekFile, long startByte) throws IOException {
		...
		// Read from here into the byte buffer , doesnt move location of
		// filepointer
		...
		try {
			do {
				...
				// MP3File.logger.finest("fc:"+fc.position() +
				// "bb"+bb.position());
				if (MPEGFrameHeader.isMPEGFrame(bb)) {
					try {
						...
						if (XingFrame.isXingFrame(bb, mp3FrameHeader)) {
							...
							try {
								// Parses Xing frame without modifying position
								// of main buffer
								mp3XingFrame = XingFrame.parseXingFrame();
							} catch (InvalidAudioFrameException ex) {
								// We Ignore because even if Xing Header is
								// corrupted
								// doesn't mean file is corrupted
							}
							break;
						}
						// There is a small but real chance that an
						// unsynchronised ID3 Frame could fool the MPEG
						// Parser into thinking it was an MPEG Header. If this
						// happens the chances of the next bytes
						// forming a Xing frame header are very remote. On the
						// basis that most files these days have
						// Xing headers we do an additional check for when an
						// apparent frame header has been found
						// but is not followed by a Xing Header:We check the
						// next header this wont impose a large
						// overhead because wont apply to most Mpegs anyway (
						// Most likely to occur if audio
						// has an APIC frame which should have been
						// unsynchronised but has not been) , or if the frame
						// has been encoded with as Unicode LE because these
						// have a BOM of 0xFF 0xFE
						else {
	...
	/**
	 * Called in some circumstances to check the next frame to ensure we have
	 * the correct audio header
	 *
	...
	private boolean isNextFrameValid(File seekFile, long filePointerCount, ByteBuffer bb, FileChannel fc)
		...
		// Our buffer is not large enough to fit in the whole of this frame,
		// something must
		// have gone wrong because frames are not this large, so just return
		// false
		// bad frame header
		...
		if (bb.remaining() <= MIN_BUFFER_REMAINING_REQUIRED + mp3FrameHeader.getFrameLength()) {
			...
			fc.read(bb, fc.position());
			bb.flip();
			// So now original buffer has been replaced, so set current position
			// to start of buffer
			currentPosition = 0;
			if (bb.limit() <= MIN_BUFFER_REMAINING_REQUIRED) {
				// No mp3 exists
	...
	/**
	 * Set number of frames in this file, use Xing if exists otherwise ((File
	 * Size - Non Audio Part)/Frame Size)
	 */
	...
	/**
	 *
	 * @return The number of frames within the Audio File, calculated as
	...
	/**
	 *
	 * @return The number of frames within the Audio File, calculated by
	 *         dividing the filesize by the number of frames, this may not be
	...
	/**
	 * Set the time each frame contributes to the audio in fractions of seconds,
	 * the higher the sampling rate the shorter the audio segment provided by
	 * the frame, the number of samples is fixed by the MPEG Version and Layer
	 */
	private void setTimePerFrame() {
		timePerFrame = mp3FrameHeader.getNoOfSamples() / mp3FrameHeader.getSamplingRate().doubleValue();

		// Because when calculating framelenggth we alter the calculation
		// slightly for MPEGVersion2
		// we seem to have to make a correspondinf modification to get the
		// correct time
	...
	/**
	 * Estimate the length of the audio track in seconds Calculation is Number
	 * of frames multiplied by the Time Per Frame using the first frame as a
	 * prototype Time Per Frame is the number of samples in the frame (which is
	 * defined by the MPEGVersion/Layer combination) divided by the sampling
	 * rate, i.e the higher the sampling rate the shorter the audio represented
	 * by the frame is going to be.
	 */
	...
	/**
	 *
	 * @return bitrate in kbps, no indicator is provided as to whether or not it
	 *         is vbr
	 */
	...
	/**
	 *
	 * @return if the bitrate is variable, Xing header takes precedence if we
	 *         have one
	 */
	...
	public String toString() {
		String s = "fileSize:" + fileSize + " encoder:" + encoder + " startByte:" + startByte + " numberOfFrames:"
				+ numberOfFrames + " numberOfFramesEst:" + numberOfFramesEstimate + " timePerFrame:" + timePerFrame
				+ " bitrate:" + bitrate + " trackLength:" + getTrackLengthAsString();
