...
import com.judy.audiotag.audio.flac.metadatablock.MetadataBlock;
import com.judy.audiotag.audio.flac.metadatablock.MetadataBlockData;
import com.judy.audiotag.audio.flac.metadatablock.MetadataBlockDataApplication;
import com.judy.audiotag.audio.flac.metadatablock.MetadataBlockDataCueSheet;
import com.judy.audiotag.audio.flac.metadatablock.MetadataBlockDataPadding;
import com.judy.audiotag.audio.flac.metadatablock.MetadataBlockDataPicture;
import com.judy.audiotag.audio.flac.metadatablock.MetadataBlockDataSeekTable;
import com.judy.audiotag.audio.flac.metadatablock.MetadataBlockDataStreamInfo;
import com.judy.audiotag.audio.flac.metadatablock.MetadataBlockHeader;
...
public class FlacTagWriter {
	...
	public void write(Tag tag, RandomAccessFile raf, RandomAccessFile rafTemp)
		...
		while (!isLastBlock) {
			...
			case PADDING:
			case PICTURE: {
				// All these will be replaced by the new metadata so we just
				// treat as padding in order
		...
		// There is enough room to fit the tag without moving the audio just
		// need to
		// adjust padding accordingly need to allow space for padding header if
		// padding required
		if ((availableRoom == neededRoom) || (availableRoom > neededRoom + MetadataBlockHeader.HEADER_LENGTH)) {
			// Jump over Flac and StreamInfoBlock
			raf.seek(FlacStream.FLAC_STREAM_IDENTIFIER_LENGTH + MetadataBlockHeader.HEADER_LENGTH
			...
			b = new byte[FlacStream.FLAC_STREAM_IDENTIFIER_LENGTH + MetadataBlockHeader.HEADER_LENGTH
					+ MetadataBlockDataStreamInfo.STREAM_INFO_DATA_LENGTH];
			raf.readFully(b);
			// Skip to start of Audio
			raf.seek(availableRoom + FlacStream.FLAC_STREAM_IDENTIFIER_LENGTH + MetadataBlockHeader.HEADER_LENGTH
					+ MetadataBlockDataStreamInfo.STREAM_INFO_DATA_LENGTH);
	...
	/**
	 *
	 * @return space currently availble for writing all Flac metadatablocks
	 *         exceprt for StreamInfo which is fixed size
	 */
	...
	/**
	 *
	 * @return space required to write the metadata blocks that are part of Flac
	 *         but are not part of tagdata in the normal sense.
	 */
