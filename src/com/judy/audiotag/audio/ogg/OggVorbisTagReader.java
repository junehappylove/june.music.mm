...
import com.judy.audiotag.tag.vorbiscomment.VorbisCommentReader;
 ...
 *
 * Vorbis is the audiostream within an ogg file, Vorbis uses VorbisComments as
 * its tag
 */
public class OggVorbisTagReader {
	 ...
	 *
	 * <p>
	 * Read the CommenyTag, within an OggVorbis file the VorbisCommentTag is
	 * mandatory
	 * 
	 * @param raf
	 * @return
	...
	/**
	 * Retrieve the Size of the VorbisComment packet including the oggvorbis
	 * header
	 *
	 * @param raf
	 * @return
	...
	/**
	 * Retrieve the raw VorbisComment packet data, does not include the
	 * OggVorbis header
	 *
	 * @param raf
	 * @return
	...
	public byte[] readRawPacketData(RandomAccessFile raf) throws CannotReadException, IOException {
		...
		// 2nd page = comment, may extend to additional pages or not , may also
		// have setup header
		pageHeader = OggPageHeader.read(raf);

		// Now at start of packets on page 2 , check this is the vorbis comment
		// header
		...
		// Convert the comment raw data which maybe over many pages back into
		// raw packet
	 ...
	 *
	 * Note this check only applies to Vorbis Comments embedded within an
	 * OggVorbis File which is why within here
	 *
	 * @param headerData
	 *
	 * @return true if the headerData matches a VorbisComment header i.e is a
	 *         Vorbis header of type COMMENT_HEADER
	 */
	public boolean isVorbisCommentHeader(byte[] headerData) {
		String vorbis = Utils.getString(headerData, VorbisHeader.FIELD_CAPTURE_PATTERN_POS,
				VorbisHeader.FIELD_CAPTURE_PATTERN_LENGTH, "ISO-8859-1");
	...
	/**
	 * The Vorbis Comment may span multiple pages so we we need to identify the
	 * pages they contain and then extract the packet data from the pages
	 */
	private byte[] convertToVorbisCommentPacket(OggPageHeader startVorbisCommentPage, RandomAccessFile raf)
		...
		// Because there is at least one other packet (SetupHeaderPacket) this
		// means the Comment Packet has finished
		...
		// There is only the VorbisComment packet on page if it has completed on
		// this page we can return
		if (!startVorbisCommentPage.isLastPacketIncomplete()) {
			logger.info("Comments finish on 2nd Page because this packet is complete");
			return baos.toByteArray();
		}

		// The VorbisComment extends to the next page, so should be at end of
		// page already
		...
		while (true) {
			...
			// Because there is at least one other packet (SetupHeaderPacket)
			// this means the Comment Packet has finished
			// on this page so thats all we need and we can return
			...
			// There is only the VorbisComment packet on page if it has
			// completed on this page we can return
	...
	/**
	 * The Vorbis Seup Header may span multiple(2) pages, athough it doesnt
	 * normally. We pass the start of the file offset of the OggPage it belongs
	 * on, it probably wont be first packet.
	 */
	public byte[] convertToVorbisSetupHeaderPacket(long fileOffsetOfStartingOggPage, RandomAccessFile raf)
		...
		// Assume that if multiple packets first packet is VorbisComment and
		// second packet
		// is setupheader
		...
		// Now should be at start of next packet, check this is the vorbis setup
		// header
		byte[] b = new byte[VorbisHeader.FIELD_PACKET_TYPE_LENGTH + VorbisHeader.FIELD_CAPTURE_PATTERN_LENGTH];
		raf.read(b);
		...
		// The Setupheader extends to the next page, so should be at end of page
		// already
		// so carry on reading pages until we get to the end of comment
		while (true) {
			...
			// Because there is at least one other packet this means the
			// Setupheader Packet has finished
			// on this page so thats all we need and we can return
			if (nextPageHeader.getPacketList().size() > 1) {
				logger.info("Setupheader finishes on this page");
				return baos.toByteArray();
			...
			// There is only the Setupheader packet on page if it has completed
			// on this page we can return
			if (!nextPageHeader.isLastPacketIncomplete()) {
	...
	public OggVorbisHeaderSizes readOggVorbisHeaderSizes(RandomAccessFile raf) throws CannotReadException, IOException {
		...
		if (!pageHeader.isLastPacketIncomplete()) {
			...
			while (true) {
				...
				if (!pageHeader.isLastPacketIncomplete()) {
					// done setup size
					logger.fine(
							"Found end of setupheader:size:" + setupHeaderSize + "finishes at:" + raf.getFilePointer());
					break;
				}
				pageHeader = OggPageHeader.read(raf);
			}
		...
		return new OggVorbisHeaderSizes(commentHeaderStartPosition, setupHeaderStartPosition, commentHeaderSize,
				setupHeaderSize);
	}

	/**
	 * Find the length of the raw packet data and the start position of the ogg
	 * page header they start in for the two OggVorbisHeader we need to know
	...
	public static class OggVorbisHeaderSizes {
		...
		OggVorbisHeaderSizes(long commentHeaderStartPosition, long setupHeaderStartPosition, int commentHeaderSize,
				int setupHeaderSize) {
		...
		/**
		 *
		 * @return the size of the raw packet data for the vorbis comment header
		 *         (includes vorbis header)
		 */
		...
		/**
		 *
		 * @return he size of the raw packet data for the vorbis setup header
		 *         (includes vorbis header)
		 */
		...
		/**
		 *
		 * @return the start position in the file of the ogg header which
		 *         contains the start of the Vorbis Comment
		 */
		 ...
		 *
		 * @return the start position in the file of the ogg header which
		 *         contains the start of the Setup Header
		 */
