...
import com.judy.audiotag.audio.ogg.OggVorbisTagReader;
import com.judy.audiotag.audio.ogg.util.OggCRCFactory;
import com.judy.audiotag.audio.ogg.util.OggPageHeader;
...
public class OggVorbisTagWriter {
	...
	public void write(Tag tag, RandomAccessFile raf, RandomAccessFile rafTemp)
		...
		rafTemp.getChannel().transferFrom(raf.getChannel(), 0, pageHeader.getPageLength()
		...
		// Compute new comment length(this may need to be spread over multiple
		// pages)
		...
		if (isCommentAndSetupHeaderFitsOnASinglePage(newCommentLength, setupHeaderLength)) {
			// And if comment and setup header originally fitted on both, the
			// length of the 2nd
			// page must be less than maximum size allowed
			// AND
			// there must be two packets with last being complete because they
			// may have
			// elected to split the data over multiple pages instead of using up
			// whole page - (as long
			...
			if ((secondPageHeader.getPageLength() < OggPageHeader.MAXIMUM_PAGE_DATA_SIZE)
				...
				replaceSecondPageOnly(oldCommentLength, setupHeaderLength, newCommentLength, newSecondPageLength,
						secondPageHeader, newComment, secondPageHeaderEndPos, raf, rafTemp);
			}
			// Original 2nd page spanned multiple pages so more work to do
			else {
				logger.info("Header and Setup now on single page:");
				replaceSecondPageAndRenumberPageSeqs(vorbisHeaderSizes, newCommentLength, newSecondPageLength,
						secondPageHeader, newComment, raf, rafTemp);
			}
		}
		// Bit more complicated, have to create a load of new pages and renumber
		// audio
		else {
			logger.info("Header and Setup remain on multiple page:");
			replacePagesAndRenumberPageSeqs(vorbisHeaderSizes, newCommentLength, secondPageHeader, newComment, raf,
					rafTemp);
		}
	}

	/**
	 * Usually can use this method, previously comment and setup header all fit
	 * on page 2 and they still do, so just replace this page. And copy further
	 * pages as is.
	 *
	 ...
	 * @throws IOException
	 */
	private void replaceSecondPageOnly(int oldCommentLength, int setupHeaderLength, int newCommentLength,
			int newSecondPageLength, OggPageHeader secondPageHeader, ByteBuffer newComment, long secondPageHeaderEndPos,
			RandomAccessFile raf, RandomAccessFile rafTemp) throws IOException {
		...
		// Build the new 2nd page header, can mostly be taken from the original
		// upto the segment length
		// OggS capture
		...
		rafTemp.getChannel().transferFrom(raf.getChannel(), rafTemp.getFilePointer(),
				raf.length() - raf.getFilePointer());

	}

	/**
	 * Previously comment and/or setup header on a number of pages now can just
	 * replace this page fitting al on 2nd page, and renumber subsequent
	 * sequence pages
	 *
	...
	private void replaceSecondPageAndRenumberPageSeqs(OggVorbisTagReader.OggVorbisHeaderSizes originalHeaderSizes,
			int newCommentLength, int newSecondPageLength, OggPageHeader secondPageHeader, ByteBuffer newComment,
		...
		// Now find the setupheader which is on a different page (or extends
		// over pages) and get it with
		// page header stripped out (it has already been worked it will all fir
		// in so should be no buffer overflow)
		byte[] setupHeaderData = reader
				.convertToVorbisSetupHeaderPacket(originalHeaderSizes.getSetupHeaderStartPosition(), raf);
		...
		// Now the Page Sequence Number for all the subsequent pages (containing
		// audio frames) are out because there are
		...
		while (raf.getFilePointer() < raf.length()) {
			...
			// Create buffer large enough foor next page (header and data) and
			// set byte order to LE so we can use
			// putInt method
			ByteBuffer nextPageHeaderBuffer = ByteBuffer
					.allocate(nextPage.getRawHeaderData().length + nextPage.getPageLength());
	...
	private void replacePagesAndRenumberPageSeqs(OggVorbisTagReader.OggVorbisHeaderSizes originalHeaderSizes,
			int newCommentLength, OggPageHeader secondPageHeader, ByteBuffer newComment, RandomAccessFile raf,
		...
		if (!isCommentAndSetupHeaderFitsOnASinglePage(lastPageCommentPacketSize,
			...
			// This page contains last bit of comment header and part of setup
			// header
			byte[] commentSegmentTable = createSegments(lastPageCommentPacketSize, true);

			// Cant just add minus the packet data from the max packet size
			// because space available to header affected
			// by size of last comment segment. i.e if it is 1 then 254 is not
			// avilable from the max size
			...
			int lastCommentHeaderLength = OggPageHeader.OGG_PAGE_HEADER_FIXED_LENGTH + commentSegmentTable.length
					+ firstHalfofSegmentHeaderTable.length;

			ByteBuffer lastCommentHeaderBuffer = ByteBuffer
					.allocate(lastCommentHeaderLength + lastPageCommentPacketSize + firstHalfOfHeaderSize);

			// Build the last comment page header
			lastCommentHeaderBuffer.put(secondPageHeader.getRawHeaderData(), 0,
			...
			byte[] setupHeaderData = reader
					.convertToVorbisSetupHeaderPacket(originalHeaderSizes.getSetupHeaderStartPosition(), raf);
			logger.finest(setupHeaderData.length + ":" + lastCommentHeaderBuffer.position() + ":"
					+ lastCommentHeaderBuffer.capacity());
			...
			// Continuous
			lastCommentHeaderBuffer.put(OggPageHeader.FIELD_HEADER_TYPE_FLAG_POS,
					OggPageHeader.HeaderTypeFlag.CONTINUED_PACKET.getFileValue());
			...
			// Transfer the second page bytebuffer content and write to temp
			// file
			...
			lastSetupHeaderBuffer.put(secondPageHeader.getRawHeaderData(), 0,
					OggPageHeader.OGG_PAGE_HEADER_FIXED_LENGTH - 1);
			...
			logger.finest(setupHeaderData.length - copyAmount + ":" + lastSetupHeaderBuffer.position() + ":"
					+ lastSetupHeaderBuffer.capacity());
			...
			// Continuous
			lastSetupHeaderBuffer.put(OggPageHeader.FIELD_HEADER_TYPE_FLAG_POS,
					OggPageHeader.HeaderTypeFlag.CONTINUED_PACKET.getFileValue());
		...
		// End of Comment and SetupHeader can fit on one page
			...
			byte[] segmentTable = createSegmentTable(lastPageCommentPacketSize,
					originalHeaderSizes.getSetupHeaderSize());
			int lastHeaderLength = OggPageHeader.OGG_PAGE_HEADER_FIXED_LENGTH + segmentTable.length;
			ByteBuffer lastCommentHeaderBuffer = ByteBuffer
					.allocate(lastHeaderLength + lastPageCommentPacketSize + originalHeaderSizes.getSetupHeaderSize());

			// Build the last comment page header base on original
			// secondpageheader
			// OggS capture
			lastCommentHeaderBuffer.put(secondPageHeader.getRawHeaderData(), 0,
					OggPageHeader.OGG_PAGE_HEADER_FIXED_LENGTH - 1);
			...
			// Add setup Header (although it will fit in this page, it may be
			// over multiple pages in its original form
		...
		long startAudioWritten = rafTemp.getFilePointer();
		logger.fine(
				"Writing audio, audio starts in original file at :" + startAudio + ":Written to:" + startAudioWritten);
		while (raf.getFilePointer() < raf.length()) {
	 ...
	 *
	 * @param newCommentLength
	 *            Of this value the start of the segment table will be created.
	 * @param setupHeaderLength
	 *            The length of setup table, zero if comment String extends over
	 *            multiple pages and this is not the last page.
	...
	private byte[] createSegmentTable(int newCommentLength, int setupHeaderLength) {
		...
		if (setupHeaderLength == 0) {
			// Comment Stream continues onto next page so last lacing value can
			// be 255
		...
		// TODO Why false, what about is header length fixed, cvant seto to true
		// because may go onto
		// next page.
	...
	/**
	 * This method creates a byte array of values whose sum should be the value
	 * of <code>length</code>.<br>
	 *
	 * @param length
	 *            Size of the page which should be represented as 255 byte
	 *            packets.
	 * @param quitStream
	 *            If true and a length is a multiple of 255 we need another
	 *            segment table entry with the value of 0. Else it's the last
	 *            stream of the table which is already ended.
	 * @return Array of packet sizes. However only the last packet will differ
	 *         from 255.
	 *
	 *         TODO if pass is data of max length (65025 bytes) and have
	 *         quitStream==true this will return 256 segments which is illegal,
	 ...
	 * @param commentLength
	 * @param setupHeaderLength
	 * @return true if there is enough room to fit the comment and the setup
	 *         headers on one page taking into account the maximum no of segment
	 *         s allowed per page and zero lacing values.
	 */
	private boolean isCommentAndSetupHeaderFitsOnASinglePage(int commentLength, int setupHeaderLength) {
		...
		if (((commentLength / OggPageHeader.MAXIMUM_SEGMENT_SIZE)
				+ (setupHeaderLength / OggPageHeader.MAXIMUM_SEGMENT_SIZE) + additionalZeroLacingRequiredForComment
				+ additionalZeroLacingRequiredForHeader) <= OggPageHeader.MAXIMUM_NO_OF_SEGMENT_SIZE) {
			return true;
		}
		return false;
	}
