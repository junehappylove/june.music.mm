...
import com.judy.audiotag.audio.exceptions.ModifyVetoException;
...
public abstract class AudioFileWriter {
	...
	public synchronized void delete(AudioFile f) throws CannotReadException, CannotWriteException {
		...
		try {
			...
			try {
				...
				System.err.println("AudioFileWriter:113:\"" + f.getFile().getAbsolutePath() + "\" or \""
	 ...
	 *             if anything went wrong
	 */
	public synchronized void delete(RandomAccessFile raf, RandomAccessFile tempRaf)
	 ...
	 */
	protected abstract void deleteTag(RandomAccessFile raf, RandomAccessFile tempRaf)
			throws CannotReadException, CannotWriteException, IOException;
	...
	public synchronized void write(AudioFile af) throws CannotWriteException {
		...
		try {
			...
			// is safe but would impose a performance
			...
			try {
				...
				// If the tempoaray file was used and there were no problems
				// replace original file with it
				if (!cannotWrite && tempF.length() > 0) {
					af.getFile().delete();
					tempF.renameTo(af.getFile());
					result = tempF;
				}
				// Either the original file was directly written to or the temp
				// file was used but the write failed
				// in either case delete the temp file.
				else {
					tempF.delete();
				}
			} catch (Exception ex) {
				System.err.println("AudioFileWriter:165:\"" + af.getFile().getAbsolutePath() + "\" or \""
						+ tempF.getAbsolutePath() + "\" :" + ex);
			}
	 ...
	 */
	protected abstract void writeTag(Tag tag, RandomAccessFile raf, RandomAccessFile rafTemp)
			throws CannotReadException, CannotWriteException, IOException;
