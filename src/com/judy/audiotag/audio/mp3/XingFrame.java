 ...
 *
 * <p>
 * In some MP3s which variable bit rate the first frame in the file contains a
 * special frame called a Xing Frame, instead of audio data. This is used to
 * store additional information about the file. The most important aspect for
 * this library is details allowing us to determine the bitrate of a Variable
 * Bit Rate VBR file without having to process the whole file.
 *
 * Xing VBR Tag data format is 120 bytes long 4 bytes for Header Tag 4 bytes for
 * Header Flags 4 bytes for FRAME SIZE 4 bytes for AUDIO_SIZE 100 bytes for
 * entry (NUMTOCENTRIES) 4 bytes for VBR SCALE. a VBR quality indicator: 0=best
 * 100=worst
 *
 * It my then contain a Lame Frame ( a Lame frame is in essence an extended Xing
 * Frame
 */
public class XingFrame {
	...
	public static final int MAX_BUFFER_SIZE_NEEDED_TO_READ_XING = MPEG_VERSION_1_MODE_STEREO_OFFSET
	...
	private XingFrame() {
		...
		// Look for LAME Header as long as we have enough bytes to do it
		// properly
	...
	/**
	 * Set whether or not VBR, (Xing can also be used for CBR though this is
	 * less useful)
	 */
	private void setVbr() {
		// Is it VBR or CBR
	...
	/**
	 * Parse the XingFrame of an MP3File, cannot be called until we have
	 * validated that this is a XingFrame
	 *
	 * @return
	...
	public static boolean isXingFrame(ByteBuffer bb, MPEGFrameHeader mpegFrameHeader) {
		...
		// Get to Start of where Xing Frame Should be ( we dont know if it is
		// one at this point)
	...
	public String toString() {
		return "xingheader" + " vbr:" + vbr + " frameCountEnabled:" + isFrameCountEnabled + " frameCount:" + frameCount
				+ " audioSizeEnabled:" + isAudioSizeEnabled + " audioFileSize:" + audioSize;
	}
}
