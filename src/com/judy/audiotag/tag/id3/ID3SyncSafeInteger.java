 ...
 *
 * <p>
 * Syncsafe integers are used for the size in the tag header of v23 and v24
 * tags, and in the frame size in the frame header of v24 frames.
 *
 * <p>
 ...
 * is particularly problematic with size descriptors. The solution in ID3v2 is
 * to use synchsafe integers, in which there can never be any false synchs.
 ...
 * Example:
 *
 * 255 (%11111111) encoded as a 16 bit synchsafe integer is 383 (%00000001
 * 01111111).
 */
public class ID3SyncSafeInteger {
	...
	/**
	 * Sizes equal or smaller than this are the same whether held as sync safe
	 * integer or normal integer so it doesnt matter.
	 */
	public static final int MAX_SAFE_SIZE = 127;

	/**
	 * Read syncsafe value from byteArray in format specified in spec and
	 * convert to int.
	 *
	 * @param buffer
	 *            syncsafe integer
	...
	private static int bufferToValue(byte[] buffer) {
		// Note Need to && with 0xff otherwise if value is greater than 128 we
		// get a negative number
		// when cast byte to int
		return (int) ((buffer[0] & 0xff) << 21) + ((buffer[1] & 0xff) << 14) + ((buffer[2] & 0xff) << 7)
				+ ((buffer[3]) & 0xff);
	}

	/**
	 * Read syncsafe value from buffer in format specified in spec and convert
	 * to int.
	 *
	 * The buffers position is moved to just after the location of the syscsafe
	 * integer
	 *
	 * @param buffer
	 *            syncsafe integer
	 ...
	 *
	 * We cannot guarantee a buffer is holding a syncsafe integer but there are
	 * some checks we can do to show that it definently is not.
	 *
	 * The buffer is NOT moved after reading.
	 *
	 * This function is useful for reading ID3v24 frames created in iTunes
	 * because iTunes does not use syncsafe integers in its frames.
	 *
	 * @param buffer
