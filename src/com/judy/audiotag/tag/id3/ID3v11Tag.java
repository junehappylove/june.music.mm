...
import com.judy.audiotag.tag.id3.framebody.FrameBodyTCON;
...
import com.judy.audiotag.tag.id3.framebody.FrameBodyTRCK;
...
public class ID3v11Tag extends ID3v1Tag {
	 ...
	 */
	private static final long serialVersionUID = 13435466576L;
			...
			if (mp3tag instanceof ID3v1Tag) {
				...
				if (id3tag.hasFrame(ID3v24Frames.FRAME_ID_GENRE)) {
					...
					try {
						...
						logger.log(Level.WARNING, getLoggingFilename() + ":"
				...
				if (id3tag.hasFrame(ID3v24Frames.FRAME_ID_TRACK)) {
					...
					try {
						...
						logger.log(Level.WARNING, getLoggingFilename() + ":"
								+ "Unable to convert TRCK frame to format suitable for v11 tag", ex);
	...
	public void setComment(String comment) {
		this.comment = ID3Tags.truncate(comment, ID3v11Tag.FIELD_COMMENT_LENGTH);
	}
	...
	/**
	 * Set the track, v11 stores track numbers in a single byte value so can
	 * only handle a simple number in the range 0-255.
	 *
	 * @param trackValue
	 */
	@Override
	...
	/**
	 * Find identifer within byteBuffer to indicate that a v11 tag exists within
	 * the buffer
	 * 
	 * @param byteBuffer
	...
	public boolean seek(ByteBuffer byteBuffer) {
		...
		byteBuffer.position(ID3v11Tag.FIELD_TRACK_INDICATOR_POS);
		if (byteBuffer.get() != END_OF_FIELD) {
			return false;
		}
		// Now check for TRACK if the next byte is also null byte then not v1.1
		// tag, however this means cannot have v1_1 tag with track set to
		// zero/undefined
	...
	public void read(ByteBuffer byteBuffer) throws TagNotFoundException {
		...
		title = Utils.getString(dataBuffer, FIELD_TITLE_POS, ID3v11Tag.FIELD_TITLE_LENGTH, "ISO-8859-1").trim();
		Matcher m = endofStringPattern.matcher(title);
		if (m.find() == true) {
			title = title.substring(0, m.start());
		}
		artist = Utils.getString(dataBuffer, FIELD_ARTIST_POS, ID3v11Tag.FIELD_ARTIST_LENGTH, "ISO-8859-1").trim();
		m = endofStringPattern.matcher(artist);
		if (m.find() == true) {
			artist = artist.substring(0, m.start());
		}
		album = Utils.getString(dataBuffer, FIELD_ALBUM_POS, ID3v11Tag.FIELD_ALBUM_LENGTH, "ISO-8859-1").trim();
		m = endofStringPattern.matcher(album);
		if (m.find() == true) {
			album = album.substring(0, m.start());
		}
		year = Utils.getString(dataBuffer, FIELD_YEAR_POS, ID3v11Tag.FIELD_YEAR_LENGTH, "ISO-8859-1").trim();
		m = endofStringPattern.matcher(year);
		if (m.find() == true) {
			year = year.substring(0, m.start());
		}
		comment = Utils.getString(dataBuffer, FIELD_COMMENT_POS, ID3v11Tag.FIELD_COMMENT_LENGTH, "ISO-8859-1").trim();
	...
	public void write(RandomAccessFile file) throws IOException {
		...
		System.arraycopy(TAG_ID, ID3v11Tag.FIELD_TAGID_POS, buffer, ID3v11Tag.FIELD_TAGID_POS, TAG_ID.length);
		int offset = ID3v11Tag.FIELD_TITLE_POS;
		if (TagOptionSingleton.getInstance().isId3v1SaveTitle()) {
			str = ID3Tags.truncate(title, ID3v11Tag.FIELD_TITLE_LENGTH);
			for (i = 0; i < str.length(); i++) {
				buffer[i + offset] = (byte) str.charAt(i);
			}
		}
		offset = ID3v11Tag.FIELD_ARTIST_POS;
		if (TagOptionSingleton.getInstance().isId3v1SaveArtist()) {
			str = ID3Tags.truncate(artist, ID3v11Tag.FIELD_ARTIST_LENGTH);
			for (i = 0; i < str.length(); i++) {
				buffer[i + offset] = (byte) str.charAt(i);
			}
		}
		offset = ID3v11Tag.FIELD_ALBUM_POS;
		if (TagOptionSingleton.getInstance().isId3v1SaveAlbum()) {
			str = ID3Tags.truncate(album, ID3v11Tag.FIELD_ALBUM_LENGTH);
			for (i = 0; i < str.length(); i++) {
				buffer[i + offset] = (byte) str.charAt(i);
			}
		}
		offset = ID3v11Tag.FIELD_YEAR_POS;
		if (TagOptionSingleton.getInstance().isId3v1SaveYear()) {
			str = ID3Tags.truncate(year, ID3v11Tag.FIELD_YEAR_LENGTH);
			for (i = 0; i < str.length(); i++) {
				buffer[i + offset] = (byte) str.charAt(i);
			}
		}
		offset = ID3v11Tag.FIELD_COMMENT_POS;
		if (TagOptionSingleton.getInstance().isId3v1SaveComment()) {
			str = ID3Tags.truncate(comment, ID3v11Tag.FIELD_COMMENT_LENGTH);
			for (i = 0; i < str.length(); i++) {
				buffer[i + offset] = (byte) str.charAt(i);
			}
		}
		offset = ID3v11Tag.FIELD_TRACK_POS;
		buffer[offset] = track; // skip one byte extra blank for 1.1 definition
		offset = ID3v11Tag.FIELD_GENRE_POS;
