...
public class ID3v22Tag extends AbstractID3v2Tag {
	 ...
	 */
	private static final long serialVersionUID = 4372887756187832893L;
	...
	protected void copyFrames(AbstractID3v2Tag copyObject) {
		...
		while (iterator.hasNext()) {
			...
			if (o instanceof AbstractID3v2Frame) {
				...
				// Special case v24 TDRC (FRAME_ID_YEAR) may need converting to
				// multiple frames
	...
	public ID3v22Tag(AbstractTag mp3tag) {
		...
		if (mp3tag != null) {
			...
			// Any tag (e.g lyrics3 and idv1.1,idv2.3 can be converted to id32.4
			// so do that
			// to simplify things
			else {
	...
	protected void readFrames(ByteBuffer byteBuffer, int size) {
		 ...
		 ** frame so should we allow for this when reading first frame, but not
		 * subsequent frames
		 */
	 ...
	 * frames, currently required for TDRC frames.
	 */
