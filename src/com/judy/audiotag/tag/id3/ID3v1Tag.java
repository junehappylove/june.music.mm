...
public class ID3v1Tag extends AbstractID3v1Tag implements Tag {
	 ...
	 */
	private static final long serialVersionUID = 124587963L;

	static EnumMap<TagFieldKey, ID3v1FieldKey> tagFieldToID3v1Field = new EnumMap<TagFieldKey, ID3v1FieldKey>(
			TagFieldKey.class);
	...
	public void setAlbum(String album) {
		this.album = ID3Tags.truncate(album, ID3v1Tag.FIELD_ALBUM_LENGTH);
	}
	 ...
	 * <p/>
	 * <p>
	 * Only one artist can be added so if one already exists it will be
	 * replaced.
	 *
	 * @param artist
	 */
	...
	public void setArtist(String artist) {
		this.artist = ID3Tags.truncate(artist, ID3v1Tag.FIELD_ARTIST_LENGTH);
	}
	 ...
	 * <p/>
	 * <p>
	 * Only one comment can be added so if one already exists it will be
	 * replaced.
	 *
	 * @param comment
	 */
	...
	public void setComment(String comment) {
		this.comment = ID3Tags.truncate(comment, ID3v1Tag.FIELD_COMMENT_LENGTH);
	}
	 ...
	 * <p/>
	 * <p>
	 * ID3v1 only supports genres defined in a predefined list so if unable to
	 * find value in list set 255, which seems to be the value winamp uses for
	 * undefined.
	 *
	 * @param genreVal
	 */
	...
	public String getFirstGenre() {
		Integer genreId = genre & ID3v1Tag.BYTE_TO_UNSIGNED;
	...
	public void setTitle(String title) {
		this.title = ID3Tags.truncate(title, ID3v1Tag.FIELD_TITLE_LENGTH);
	}
	...
	public void setYear(String year) {
		this.year = ID3Tags.truncate(year, ID3v1Tag.FIELD_YEAR_LENGTH);
	}
	...
	public void read(ByteBuffer byteBuffer) throws TagNotFoundException {
		...
		title = Utils.getString(dataBuffer, FIELD_TITLE_POS, ID3v1Tag.FIELD_TITLE_LENGTH, "ISO-8859-1").trim();
		Matcher m = endofStringPattern.matcher(title);
		if (m.find() == true) {
			title = title.substring(0, m.start());
		}
		artist = Utils.getString(dataBuffer, FIELD_ARTIST_POS, ID3v1Tag.FIELD_ARTIST_LENGTH, "ISO-8859-1").trim();
		m = endofStringPattern.matcher(artist);
		if (m.find() == true) {
			artist = artist.substring(0, m.start());
		}
		album = Utils.getString(dataBuffer, FIELD_ALBUM_POS, ID3v1Tag.FIELD_ALBUM_LENGTH, "ISO-8859-1").trim();
		...
		year = Utils.getString(dataBuffer, FIELD_YEAR_POS, ID3v1Tag.FIELD_YEAR_LENGTH, "ISO-8859-1").trim();
		m = endofStringPattern.matcher(year);
		if (m.find() == true) {
			year = year.substring(0, m.start());
		}
		comment = Utils.getString(dataBuffer, FIELD_COMMENT_POS, ID3v1Tag.FIELD_COMMENT_LENGTH, "ISO-8859-1").trim();
		...
		genre = dataBuffer[ID3v1Tag.FIELD_GENRE_POS];
	...
	public void write(RandomAccessFile file) throws IOException {
		...
		System.arraycopy(TAG_ID, ID3v1Tag.FIELD_TAGID_POS, buffer, ID3v1Tag.FIELD_TAGID_POS, TAG_ID.length);
		int offset = ID3v1Tag.FIELD_TITLE_POS;
		if (TagOptionSingleton.getInstance().isId3v1SaveTitle()) {
			str = ID3Tags.truncate(title, ID3v1Tag.FIELD_TITLE_LENGTH);
			for (i = 0; i < str.length(); i++) {
				buffer[i + offset] = (byte) str.charAt(i);
			}
		}
		offset = ID3v1Tag.FIELD_ARTIST_POS;
		if (TagOptionSingleton.getInstance().isId3v1SaveArtist()) {
			str = ID3Tags.truncate(artist, ID3v1Tag.FIELD_ARTIST_LENGTH);
			for (i = 0; i < str.length(); i++) {
				buffer[i + offset] = (byte) str.charAt(i);
			}
		}
		offset = ID3v1Tag.FIELD_ALBUM_POS;
		if (TagOptionSingleton.getInstance().isId3v1SaveAlbum()) {
			str = ID3Tags.truncate(album, ID3v1Tag.FIELD_ALBUM_LENGTH);
			for (i = 0; i < str.length(); i++) {
				buffer[i + offset] = (byte) str.charAt(i);
			}
		}
		offset = ID3v1Tag.FIELD_YEAR_POS;
		...
		offset = ID3v1Tag.FIELD_COMMENT_POS;
		if (TagOptionSingleton.getInstance().isId3v1SaveComment()) {
			str = ID3Tags.truncate(comment, ID3v1Tag.FIELD_COMMENT_LENGTH);
			for (i = 0; i < str.length(); i++) {
				buffer[i + offset] = (byte) str.charAt(i);
			}
		}
		offset = ID3v1Tag.FIELD_GENRE_POS;
