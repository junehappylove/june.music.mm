 ...
 * @author : Paul Taylor
 * @author : Eric Farng
 * @version $Id: AbstractID3v2Frame.java,v 1.24 2007/11/26 14:20:28 paultaylor
 *          Exp $
 */
public abstract class AbstractID3v2Frame extends AbstractTagFrame implements TagField {
	...
	// The purpose of this is to provide the filename that should be used when
	// writing debug messages
	// when problems occur reading or writing to file, otherwise it is difficult
	...
	public AbstractID3v2Frame(String identifier) {
		...
		// Use reflection to map id to frame body, which makes things much
		// easier
	...
	/**
	 * Return the frame identifier, this only identifiies the frame it does not
	 * provide a unique key, when using frames such as TXXX which are used by
	 * many fields *
	 *
	 * @return the frame identifier (Tag Field Interface)
	 */
	// TODO, this is confusing only returns the frameId, which does not
	// neccessarily uniquely
	// identify the frame
	public String getId() {
	...
	protected AbstractID3v2FrameBody readBody(String identifier, ByteBuffer byteBuffer, int frameSize)
		...
		try {
			Class<?> c = Class.forName("com.judy.audiotag.tag.id3.framebody.FrameBody" + identifier);
			Class<?>[] constructorParameterTypes = { ((Class<?>) Class.forName("java.nio.ByteBuffer")), Integer.TYPE };
		...
		// No class defined for this frame type,use FrameUnsupported
			...
			logger.info(getLoggingFilename() + ":" + "Identifier not recognised:" + identifier
					+ " using FrameBodyUnsupported");
			try {
				frameBody = new FrameBodyUnsupported(byteBuffer, frameSize);
			}
			// Should only throw InvalidFrameException but unfortunately legacy
			// hierachy forces
		...
		// An error has occurred during frame instantiation, if underlying cause
		// is an unchecked exception or error
	...
	/**
	 * This creates a new body based of type identifier but populated by the
	 * data in the body. This is a different type to the body being created
	 * which is why TagUtility.copyObject() can't be used. This is used when
	 * converting between different versions of a tag for frames that have a
	 * non-trivial mapping such as TYER in v3 to TDRC in v4. This will only work
	 * where appropriate constructors exist in the frame body to be created, for
	 * example a FrameBodyTYER requires a constructor consisting of a
	 * FrameBodyTDRC.
	 *
	 * If this method is called and a suitable constructor does not exist then
	 * an InvalidFrameException will be thrown
	 *
	 * @param identifier
	 *            to determine type of the frame
	 * @throws InvalidFrameException
	 *             if unable to construct a framebody for the identifier and
	 *             body provided.
	...
	protected AbstractID3v2FrameBody readBody(String identifier, AbstractID3v2FrameBody body)
		...
		/*
		 * Use reflection to map id to frame body, which makes things much
		 * easier to keep things up to date, although slight performance hit.
		 */
		AbstractID3v2FrameBody frameBody;
		try {
			Class<?> c = Class.forName("com.judy.audiotag.tag.id3.framebody.FrameBody" + identifier);
	...
	public void isBinary(boolean b) {
		// do nothing because whether or not a field is binary is defined by its
		// id and is immutable
	}
