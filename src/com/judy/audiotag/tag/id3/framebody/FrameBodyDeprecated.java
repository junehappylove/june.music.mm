...
/**
 * Represents a framebody for a frame identifier that is not defined for the tag
 * version but was valid for a for an earlier tag version. The body consists of
 * an array of bytes representing all the bytes in the body.
 */
public class FrameBodyDeprecated extends AbstractID3v2FrameBody implements ID3v24FrameBody, ID3v23FrameBody {
	/*
	 * The original framebody is held so can be retrieved when converting a
	 * DeprecatedFrameBody back to a normal Framebody
	 */
	...
	/**
	 * Delgate size to size of original framebody, if framebody already exist
	 * will take this value from the frame header but it is always recalculated
	 * before writing any changes back to disk.
	 *
	...
	/**
	 * Return the original framebody that was used to construct the
	 * DeprecatedFrameBody
	 *
