...
import com.judy.audiotag.tag.datatype.AbstractDataType;
...
/**
 * Contains the content for an ID3v2 frame, (the header is held directly within
 * the frame
 */
public abstract class AbstractID3v2FrameBody extends AbstractTagFrameBody {
	...
	/**
	 * Creates a new FrameBody datatype from file. The super Constructor sets up
	 * the Object list for the frame.
	 *
	 * @param byteBuffer
	...
	/**
	 * Return the ID3v2 Frame Identifier, must be implemented by concrete
	 * subclasses
	 *
	...
	/**
	 * Return size of frame body,if framebody already exist will take this value
	 * from the frame header but it is always recalculated before writing any
	...
	/**
	 * Set size based on size passed as parameter from frame header, done before
	 * read
	 */
	...
	/**
	 * Set size based on size of the DataTypes making up the body,done after
	 * write
	 */
	public void setSize() {
		size = 0;
	...
	/**
	 * This reads a frame body from a ByteBuffer into the appropriate FrameBody
	 * class and update the position of the buffer to be just after the end of
	 * this framebody
	 *
	 * The ByteBuffer represents the tag and its position should be at the start
	 * of this framebody. The size as indicated in the header is passed to the
	 * frame constructor when reading from file.
	 *
	 * @param byteBuffer
	 *            file to read
	 *
	...
	public void read(ByteBuffer byteBuffer) throws InvalidTagException {
		...
		// this offset is only used internally to decide where to look for the
		// next
		// datatype within a framebody, it does not decide where to look for the
		// next frame body
		int offset = 0;
		...
		for (AbstractDataType object : objectList) {
			...
			// The read has extended further than the defined frame size (ok to
			// extend upto
		...
		for (AbstractDataType object : objectList) {
			...
			if (objectData != null) {
				try {
					...
					// This could never happen coz not writing to file, so
