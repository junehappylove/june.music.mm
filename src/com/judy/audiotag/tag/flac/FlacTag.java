package com.judy.audiotag.tag.flac;
...
/**
 * Flac uses Vorbis Comment for most of its metadata and a Flac Picture Block
 * for images
 *
 * <p>
...
public class FlacTag implements Tag {
	 ...
	 * {@link com.judy.jaudiotagger.tag.TagField} implementation does not return
	 * a text field compliant data with
	 * {@link com.judy.jaudiotagger.tag.TagField#getRawContent()} other software
	 * and the audio library won't read the file correctly, if they do read it
	 * at all. <br>
	 * So for short:<br>
	 * <uil>
	 ...
	 *
	 * <p>
	 * If there are no images we return empty if either there is no VorbisTag or
	 * if there is a VorbisTag but it is empty
	 *
	 ...
	 * @throws FieldDataInvalidException
	 */
	public TagField createArtworkField(byte[] imageData, int pictureType, String mimeType, String description,
			int width, int height, int colourDepth, int indexedColouredCount) throws FieldDataInvalidException {
		return new MetadataBlockDataPicture(imageData, pictureType, mimeType, description, width, height, colourDepth,
	 ...
	 * @throws FieldDataInvalidException
	 */
	public TagField createArtworkField(BufferedImage bi, int pictureType, String mimeType, String description,
			int colourDepth, int indexedColouredCount) throws FieldDataInvalidException {
		...
		try {
			...
			// Add to image list
			return new MetadataBlockDataPicture(output.toByteArray(), pictureType, mimeType, description, bi.getWidth(),
					bi.getHeight(), colourDepth, indexedColouredCount);
	...
	/**
	 * Create Link to Image File, not recommended because if either flac or
	 * image file is moved link will be broken.
	 */
	public TagField createLinkedArtworkField(String url) {
		...
		return new MetadataBlockDataPicture(Utils.getDefaultBytes(url, TextEncoding.CHARSET_ISO_8859_1),
				PictureTypes.DEFAULT_ID, MetadataBlockDataPicture.IMAGE_IS_URL, "", 0, 0, 0, 0);
	}
}
