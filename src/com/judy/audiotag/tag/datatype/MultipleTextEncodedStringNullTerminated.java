...
/**
 * Represents a datatype that supports multiple terminated Strings (there may
 * only be one)
 */
public class MultipleTextEncodedStringNullTerminated extends AbstractDataType {
	...
	/**
	 * Read Null Terminated Strings from the array starting at offset, continue
	 * until unable to find any null terminated Strings or until reached the end
	 * of the array. The offset should be set to byte after the last null
	 * terminated String found.
	 *
	 * @param arr
	 ...
	 *
	 * @return bytebuffer that should be written to file to persist this
	 *         datatype.
	 */
	public byte[] writeByteArray() {
		...
		try {
			...
			// This should never happen because the write is internal with the
			// JVM it is not to a file
			logger.log(Level.SEVERE, "IOException in MultipleTextEncodedStringNullTerminated when writing byte array",
					ioe);
