...
public class StringFixedLength extends AbstractString {
	...
	/**
	 * Read a string from buffer of fixed size(size has already been set in
	 * constructor)
	 *
	 * @param arr
	...
	public void readByteArray(byte[] arr, int offset) throws InvalidDataTypeException {
		...
		try {
			...
			// Decode buffer if runs into problems should through exception
			// which we
	 ...
	 *
	 * The string will be adjusted to ensure the correct number of bytes are
	 * written, If the current value is null or to short the written value will
	 * have the 'space' character appended to ensure this. We write this instead
	 * of the null character because the null character is likely to confuse the
	 * parser into misreading the next field.
	 *
	...
	public byte[] writeByteArray() {
		...
		// Create with a series of empty of spaces to try and ensure integrity
		// of field
		if (value == null) {
		...
		try {
			...
			logger.warning("There was a problem writing the following StringFixedlength Field:" + value + ":"
					+ ce.getMessage() + "using default value instead");
			data = new byte[size];
			for (int i = 0; i < size; i++) {
				data[i] = ' ';
			}
		...
		if (dataBuffer != null) {
			...
			// There is more data available than allowed for this field strip
				...
				logger.warning("There was a problem writing the following StringFixedlength Field:" + value
						...
						+ " but field was defined with length of:" + size + " too long so stripping extra length");
				data = new byte[size];
			...
			logger.warning("There was a serious problem writing the following StringFixedlength Field:" + value + ":"
