...
/**
 * Represents a number which may span a number of bytes when written to file
 * depending what size is to be represented.
 *
 ...
 *
 * In ID3Specification would be denoted as $xx xx xx xx (xx ...) , this denotes
 * at least four bytes but may be more. Sometimes may be completely optional
 * (zero bytes)
 */
public class NumberVariableLength extends AbstractDataType {
	...
	/**
	 * Creates a new ObjectNumberVariableLength datatype, set minimum length to
	 * zero if this datatype is optional.
	 *
	 * @param identifier
	 * @param minimumSize
	 */
	...
	/**
	 * Return the maximum number of digits that can be used to express the
	 * number
	 *
	 * @return the maximum number of digits that can be used to express the
	 *         number
	...
	/**
	 * Return the minimum number of digits that can be used to express the
	 * number
	 *
	 * @return the minimum number of digits that can be used to express the
	 *         number
	...
	public void readByteArray(byte[] arr, int offset) throws InvalidDataTypeException {
		...
		// If optional then set value to zero, this will mean that if this frame
		// is written back to file it will be created
		// with this additional datatype wheras it didnt exist but I think this
		// is probably an advantage the frame is
		// more likely to be parsed by other applications if it contains
		// optional fields.
		...
		// Read the bytes (starting from offset), the most significant byte of
		// the number being constructed is read first,
		// we then shift the resulting long one byte over to make room for the
		// next byte
	...
	public byte[] writeByteArray() {
		...
		if (size == 0) {
			...
			// keeps shifting the number downwards and masking the last 8 bist
			// to get the value for the next byte
			// to be written
