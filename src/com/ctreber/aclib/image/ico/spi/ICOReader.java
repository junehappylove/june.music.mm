...
public class ICOReader extends ImageReader {
	...
	private BitmapDescriptor getICOEntry(final int pImageIndex) {
		return (BitmapDescriptor) getICOFile().getDescriptors().get(pImageIndex);
	}
	...
	private static JFrame createWindow(final String pTitle) {
		...
		final LayoutManager lLayout = new BoxLayout(lFrame.getContentPane(), BoxLayout.Y_AXIS);
		lFrame.getContentPane().setLayout(lLayout);
		return lFrame;
	}

	private static void addImage(final Container pParent, final ImageReader pReader, final int pImageNo)
			throws IOException {
		final JButton lButton = new JButton();
		lButton.setIcon(new ImageIcon(pReader.read(pImageNo)));
		lButton.setText("" + pReader.getWidth(pImageNo) + "x" + pReader.getHeight(pImageNo));
		pParent.add(lButton);
	}
	...
	public static void listServiceProviders() {
		...
		for (int lFormatNo = 0; lFormatNo < lFormats.length; lFormatNo++) {
			...
			while (lItReader.hasNext()) {
				...
				final ImageReaderSpi lProvider = lReader.getOriginatingProvider();
				System.out.println(" o " + lFormat + " (" + lProvider.getDescription(Locale.getDefault()) + ") by "
						+ lProvider.getVendorName());
			}
		}
	}
}
